...
{
    /// <summary>
    /// Implementation for saving meet info into a database
    /// </summary>
    public class MeetDBSvcImpl : IMeetDBSvc
    {
        ...
        public Meet FindMeet(int id)
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    myMeet.performances = FindPerformances(id);
                }
            }
            catch (Exception e)
            {
        ...
        public Dictionary<string, List<Performance>> FindPerformances(int id)
        {
            ...
            {
                List<Performance> boysPerfs = FindPerformances(id, "Boy's " + str);
                if(boysPerfs.Count > 0)
                {
                    dictOfPerfs.Add("Boy's " +str, boysPerfs);
                }
                List<Performance> girlsPerfs = FindPerformances(id, "Girl's " + str);
        ...
        {
            string connectionString = WebConfigurationManager.ConnectionStrings["LookupMeetDB2"].ConnectionString;
